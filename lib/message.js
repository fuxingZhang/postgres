'use strict';

const util = require('util');

class Message {
  /**
   * @constructor
   * @param {Buffer} buffer 
   */
  constructor(buffer) {
    this.parseHeader(buffer);
  }

  parseHeader(buffer) {
    const header = buffer.toString('utf8', 0, 1);
    console.log('\r\n parseHeader ==>')
    console.log({ header });
    // console.log(buffer.toString('ascii', 9));
    console.log('<== parseHeader \r\n')
    switch (header) {
      case 'R':
        return this.parseR(buffer);
      case 'E':
        return this.parseE(buffer);
      case 'T':
        return this.parseT(buffer);
      default: 
        throw new Error(`Unprocessed header: ${header}`);
    }
  }

  parseR(buffer) {
    console.log('\r\n parseR ===>')
    let offset = 1;
    const length = buffer.readInt32BE(offset);
    offset += 4;

    const code = buffer.readInt32BE(offset);
    offset += 4;

    console.log({
      length,
      code
    });

    switch (code) {
      case 0:
        this.name = 'AuthenticationOk';
        this.message = buffer.toString('ascii', 9);
        break;
      case 3:
        if (msg.length === 8) {
          this.name = 'AuthenticationCleartextPassword';
        } else {
          this.name = `parseR code 3, length expect 8, get ${length}`;
        }
        break;
      case 5:
        if (length === 12) {
          this.name = 'AuthenticationMD5Password';
          this.salt = buffer.slice(offset, offset + 4);
        } else {
          this.name = `parseR code 5, length expect 12, get ${length}`;
        }
        break;
      default:
        throw new Error('Unknown Authentication type' + util.inspect({
          code,
          message: buffer.slice(offset, offset + 4)
        }));
    }
  }

  parseT(buffer) {
    this.name = 'RowDescription';

    let offset = 1;
    const len = buffer.readInt32BE(offset) - 4;
    offset += 4;
    console.log('parseT len:', len);

    const fieldCount = buffer.readInt16BE(offset);
    offset += 2;
    console.log('parseT fieldCount:', fieldCount);

    const fields = [];
    for(let i = 0; i< fieldCount; i++) {
      const end = buffer.indexOf(0, offset);
      const name = buffer.toString('utf8', offset, end);
      offset += end + 1;
      console.log('parseT name:', name);
  
      const tableID = buffer.readInt32BE(offset);
      offset += 4;
      console.log('parseT tableID:', tableID);
  
      const columnID = buffer.readInt16BE(offset);
      offset += 2;
      console.log('parseT columnID:', columnID);
  
      const dataTypeID = buffer.readInt32BE(offset);
      offset += 4;
      console.log('parseT dataTypeID:', dataTypeID);
  
      const dataTypeSize = buffer.readInt16BE(offset);
      offset += 2;
      console.log('parseT dataTypeSize:', dataTypeSize);
  
      const dataTypeModifier = buffer.readInt32BE(offset);
      offset += 4;
      console.log('parseT dataTypeModifier:', dataTypeModifier);
  
      const mode = buffer.readInt16BE(offset);
      offset += 2;
      console.log('parseT mode:', mode);
  
      console.log({
        offset,
        len: Buffer.byteLength(buffer)
      })
      const format = mode === 0 ? 'text' : 'binary'; // Currently will be zero (text) or one (binary).

      fields.push({
        name,
        tableID,
        columnID,
        dataTypeID,
        dataTypeSize,
        dataTypeModifier,
        format
      });
    }

    this.fields = fields;
    console.log(fields);

    const header = buffer.toString('utf8', offset);
    // const header = buffer.toString('utf8', offset);
    console.log({header})
    console.log(header)
  }

  parseE(buffer) {
    this.name = 'error';
    console.log(buffer.toString('utf8', 0, 1));
    console.log(buffer.indexOf(6, 0));
    const bufferLen = Buffer.byteLength(buffer);
    console.log({ bufferLen });
    console.log(buffer.readUInt32BE(1));
    console.log(buffer.readUInt32LE(1));
    const len = buffer.readInt32LE(1);
    console.log({ len });
    const name = buffer.toString('utf8', 5, 1);
    console.log({ name });
    const message = buffer.toString('ascii', 0, buffer.indexOf(0, 6));
    console.log({ message });
    this.message = message;
  }
}

module.exports = Message